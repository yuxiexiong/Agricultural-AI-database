...
旧长文档已作为历史底稿保留在 `archive/`。它不再是主维护入口。

已完成拆分：

- 旧长文档第 8 章素材已拆分为 `materials/` 下的 84 条独立素材卡片。
- 总索引见 `materials/index.md`。
- 每条素材卡片保留原始文档路径和行号，便于追溯。

后续工作：

- 如果旧文档第 0-7 章中还有可复用段落，应继续拆成 `reports/` 模板或 `materials/` 卡片。
- 可结构化的观点、场景、ROI 和风险，应同步补入 `database/knowledge/`。
